...
increase = [[0, 0, 0] for _ in range(N)]
decrease = [[0, 0, 0] for _ in range(N)]
increase[0] = [1, 0, arr[0]]
decrease[0] = [1, 0, reversed_arr[0]]
...
for i in range(1, N):
    ...
    if (RLIS[-1] < reversed_arr[i]):
        ...
        decrease[i] = [len(RLIS), i, reversed_arr[i]]
    else:
...
for i in range(N):
    ...
    idx_sum = increase_value[1] + decrease_value[1]

    if (idx_sum == N-1 or increase_value[2] == decrease_value[2]):
        len_sum -= 1

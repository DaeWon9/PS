...
for _ in range(party_count):
    party_attended_people_list.append(list(map(int, sys.stdin.readline().split()))[1:])

while True:
    count = 0
    for index in range(party_count):
        if not visited[index] and set(party_attended_people_list[index]).intersection(truth_know_people):
            visited[index] = True
            count += 1
            for item in set(party_attended_people_list[index]).difference(truth_know_people):
